---
name: PRISMA-ScR
full_name: PRISMA extension for Scoping Reviews
version: "2018"
extends: prisma-2020
applies_to: Scoping reviews
items: 22
...
22 items for **scoping reviews** — which map the extent, range and nature of evidence on a
topic rather than answering a narrowly framed effectiveness question.

Vocabulary differs deliberately from PRISMA 2020, and the differences are substantive:

- **"Sources of evidence"** rather than "studies" — scoping reviews may include qualitative
  research, expert opinion and policy documents.
- **"Data charting"** rather than "data extraction" (per Arksey & O'Malley, Levac et al.,
  and JBI guidance).
- **"Critical appraisal"** rather than "risk of bias", and it is *optional* (items 12, 16) —
  a scoping review need not appraise, but must say whether it did and why.

Cite as `[@tricco2018prismascr]`.
...
- **\*** Where sources of evidence (see †) are compiled from, such as bibliographic databases, social media platforms, and Web sites.
- **†** A more inclusive/heterogeneous term used to account for the different types of evidence or data sources (e.g., quantitative and/or qualitative research, expert opinion, and policy documents) that may be eligible in a scoping review as opposed to only studies. Not to be confused with *information sources* (\*).
...
- **§** The process of systematically examining research evidence to assess its validity, results, and relevance before using it to inform a decision. Used for items 12 and 19 instead of "risk of bias" (more applicable to systematic reviews of interventions) to acknowledge the various sources of evidence that may be used in a scoping review.
